...
class BranchCondition {
    ...
    async BranchConditionDone() {
        ...
        if(withoutSFirstOperand && withoutSSecondOperand &&
            ...
            const rawFirstOperand = changeType(withoutSFirstOperand, this.typeOfOperand);
            const rawSecondOperand = changeType(withoutSSecondOperand, this.typeOfOperand);
            switch (this.logicalOperation) {
                ...
                case "<=":
                    ...
                    break;
                case "includes":
                    if (rawFirstOperand && rawSecondOperand && typeof rawFirstOperand === "string"
                        && typeof rawSecondOperand === "string"
                        && rawFirstOperand.includes(rawSecondOperand)) {
                        const scrape = new Scrape(this.browserController, this.json, this.ifProgram, this.page, this.S);
                        await scrape.LaunchProgram();
                    } else {
                        const scrape = new Scrape(this.browserController, this.json, this.elseProgram, this.page, this.S);
